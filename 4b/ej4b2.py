...
def average_score_ram(scores_dict:list[float]):
    media = 0
    if not scores_dict:
        raise ValueError("El diccionario debe contener valores")
    try:
        for i in scores_dict:
            media += scores_dict[i]
        return media/len(scores_dict)
    except TypeError:
        raise TypeError("Los valores deben ser int/float")
